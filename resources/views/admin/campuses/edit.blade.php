...
@section('title', 'Modifier ' . $campus->name)
@section('page-title', 'Modifier le Campus: ' . $campus->name)

@section('content')
<div class="max-w-3xl mx-auto">
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-2xl font-bold text-gray-800">Modifier le Campus: {{ $campus->name }}</h2>
    ...
    <div class="bg-white rounded-lg shadow p-6">
        <form method="POST" action="{{ route('admin.campuses.update', $campus->id) }}">
            @csrf
            @method('PUT')
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="text" name="name" id="name" value="{{ old('name', $campus->name) }}" 
                        ...
                        <div>
                            ...
                            <input type="text" name="code" id="code" value="{{ old('code', $campus->code) }}" 
                        ...
                        <div class="md:col-span-2">
                            ...
                            <textarea name="address" id="address" rows="3" 
                                      class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 @error('address') border-red-500 @enderror">{{ old('address', $campus->address) }}</textarea>
                            @error('address')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="md:col-span-2">
                            ...
                            <textarea name="description" id="description" rows="3" 
                                      class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 @error('description') border-red-500 @enderror">{{ old('description', $campus->description) }}</textarea>
                            @error('description')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div>
                            ...
                            <input type="number" name="latitude" id="latitude" step="any" value="{{ old('latitude', $campus->latitude) }}" 
                        ...
                        <div>
                            ...
                            <input type="number" name="longitude" id="longitude" step="any" value="{{ old('longitude', $campus->longitude) }}" 
                        ...
                        <div>
                            ...
                            <select name="radius" id="radius" 
                                ...
                                <option value="50" {{ old('radius', $campus->radius) == 50 ? 'selected' : '' }}>50m</option>
                                <option value="100" {{ old('radius', $campus->radius) == 100 ? 'selected' : '' }}>100m</option>
                                <option value="150" {{ old('radius', $campus->radius) == 150 ? 'selected' : '' }}>150m</option>
                                <option value="200" {{ old('radius', $campus->radius) == 200 ? 'selected' : '' }}>200m</option>
                                <option value="300" {{ old('radius', $campus->radius) == 300 ? 'selected' : '' }}>300m</option>
                                <option value="500" {{ old('radius', $campus->radius) == 500 ? 'selected' : '' }}>500m</option>
                                <option value="1000" {{ old('radius', $campus->radius) == 1000 ? 'selected' : '' }}>1000m</option>
                                <option value="1500" {{ old('radius', $campus->radius) == 1500 ? 'selected' : '' }}>1500m</option>
                                <option value="2000" {{ old('radius', $campus->radius) == 2000 ? 'selected' : '' }}>2000m</option>
                                <option value="3000" {{ old('radius', $campus->radius) == 3000 ? 'selected' : '' }}>3000m</option>
                                <option value="5000" {{ old('radius', $campus->radius) == 5000 ? 'selected' : '' }}>5000m</option>
                            </select>
                            @error('radius')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="time" name="start_time" id="start_time" value="{{ old('start_time', $campus->start_time) }}" 
                        ...
                        <div>
                            ...
                            <input type="time" name="end_time" id="end_time" value="{{ old('end_time', $campus->end_time) }}" 
                        ...
                        <div>
                            ...
                            <input type="number" name="late_tolerance" id="late_tolerance" value="{{ old('late_tolerance', $campus->late_tolerance) }}" min="0" max="60"
...
<script>
    ...
    async function initMap() {
        const location = {
            lat: {{ old('latitude', $campus->latitude) }},
            lng: {{ old('longitude', $campus->longitude) }}
        };
        ...
        marker = mapHelper.addMarker(location.lat, location.lng, {
            draggable: true,
        ...
        updateCircle(location.lat, location.lng);
    }
    ...
    function updateCircle(lat, lng) {
        ...
        const radius = parseFloat(document.getElementById('radius').value) || {{ old('radius', $campus->radius) }};
